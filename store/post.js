...
export const usePostStore = defineStore('post', {
    ...
    actions: {
        ...
        setIsLoadingPosts(isLoadingPosts) {
        ...
        deletePost(deletePost) {
            const deletedIndex = this.loadedPosts.findIndex(
                post => post.id === deletePost
            );
        ...
        async addPost(postData) {
            ...
            try {
                const data = await this.$axios.$post(`/posts.json?auth=${authStore.token}`, createdPost);
                if (!data) return;
    
                const updatedData = { ...createdPost, id: data.name };
                await this.$axios.$put(`/posts/${data.name}.json?auth=${authStore.token}`, updatedData);
    
                this.addPost(updatedData);
                userStore.addUserPost(updatedData);
        ...
        async editPost(postData) {
            try {
                await this.$axios.$put(`/posts/${postData.id}.json?auth=${authStore.token}`, postData);
                this.editPost(postData);
                // commit("user/editUserPost", postData, { root: true });
                userStore.editUserPost(postData);
            } catch (error) {
                console.error(error);
            }
        },
        async uploadPreviewImage({ postId, previewImageFile }) {
            if (!previewImageFile) return;
        ...
        async deletePost(deletePostId) {
            ...
            try {
                await this.$axios.$delete(`/posts/${deletePostId}.json?auth=${authStore.token}`);
                await Promise.all([deleteImage(previewImgRef), deleteImage(contentImgRef)]);
                this.deletePost(deletePostId);
                userStore.deleteUserPost(deletePostId);
            } catch (error) {
                console.error(error);
            }
        },
