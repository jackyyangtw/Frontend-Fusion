<template>
  <div class="relative">
    <button
      ...
      type="button"
      class="inline-flex items-center p-2 my-3 ml-8 text-sm text-gray-500 rounded-lg bg-sky-500 dark:bg-pink-600/[0.5] xl:hidden"
    >
        ...
        <ul class="space-y-2 font-medium">
          <li @click="selectTag('全部類型')">
            <a
              ...
              :class="{
                'bg-gray-100 dark:bg-gray-700': selectedTag === '全部類型',
              }"
            >
              <span class="ml-3"
                >全部類型
                <span class="font-bold text-sky-500 dark:text-pink-500"
                  >({{ posts.length }})</span
                >
              </span>
            </a>
          ...
          <li v-for="tag in tags" :key="tag" @click="selectTag(tag)">
            <a
              href="#"
              class="flex items-center p-2 text-gray-900 rounded-lg dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700"
              :class="{ 'bg-gray-100 dark:bg-gray-700': selectedTag === tag }"
...
export default {
  ...
  mounted() {
    this.$store.commit("ui/setSidebarWidth", this.$refs.sidebar.clientWidth + 20);
  },
};
</script>